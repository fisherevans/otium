// Topic icon registry (#45, expanded #72). A topic's `icon` field stores a stable
...
//
// The set is intentionally broad so a topic can pick something close to its topic
...
//
// Keys are stable: existing topics store them, so rename/remove is a migration.
...
export interface FeedIconDef {
  key: string; // stored in topics.icon
...
export const FEED_ICONS: FeedIconDef[] = [
  ...
  { key: "podcasts", label: "Podcasts (topic)", Icon: Podcast },
  ...
  // --- generic ---
  { key: "default", label: "Generic topic", Icon: Rss },
];
...
// is unset/unknown (the caller then falls back to the topic's color swatch).
